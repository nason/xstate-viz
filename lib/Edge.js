...
var Edge = /** @class */ (function (_super) {
    ...
    function Edge() {
        ...
        _this.state = {
            ...
            sourceElement: null,
            targetElement: null,
            sourceData: tracker_1.tracker.get(_this.props.edge.source.id),
            targetData: tracker_1.tracker.get(_this.props.edge.target.id),
            eventData: tracker_1.tracker.get(utils_1.serializeEdge(_this.props.edge))
        };
    ...
    Edge.prototype.componentDidMount = function () {
        ...
        var eventId = utils_1.serializeEdge(edge);
        tracker_1.tracker.listen(eventId, function (data) {
            _this.setState({ eventData: data });
        });
        tracker_1.tracker.listen(edge.source.id, function (data) {
            _this.setState({ sourceData: data });
        });
        tracker_1.tracker.listen(edge.target.id, function (data) {
            var target = edge.target;
            var parentData = data;
            while (utils_1.isHidden(parentData.element)) {
                if (!target.parent) {
                    break;
                }
                target = target.parent;
                parentData = tracker_1.tracker.get(target.id);
                if (!parentData) {
                    break;
                }
            }
    ...
    Edge.prototype.render = function () {
        ...
        // );
        var eventRect = utils_1.relative(this.state.eventData.rect, svgRef);
        var targetRect = utils_1.relative(this.state.targetData.rect, svgRef);
        var eventCenterPt = utils_1.center(eventRect);
        var targetCenterPt = utils_1.center(targetRect);
        var start = {
            x: eventRect.right - 4,
            y: eventCenterPt.y
        };
        ...
        return (react_1.default.createElement("g", null,
            react_1.default.createElement("path", { d: path, stroke: isHighlighted ? "gray" : "var(--color-edge)", strokeWidth: strokeWidth, fill: "none", markerEnd: isHighlighted ? "url(#marker-preview)" : "url(#marker)", ref: this.ref })));
    };
    return Edge;
}(react_1.Component));
exports.Edge = Edge;
